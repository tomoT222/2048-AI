...
////      select action
////
int select_action(vector<int> Q, double eps)
{
...
int main()
{
    ...
    vector<vector<double>> V(n_tuple_cnt, vector<double>(pow(MAX2, 6)));
        ...
        rep(k, num_iteration)
        {
            ...
            rep2(times, 8)
            {
                rep(ind, n_tuple_cnt)
                {
                    ...
                    rep(i, 6)
                    {
                        BT_ind[i] = min(MAX2 - 1, agent.grid[x[ind][i]][y[ind][i]]);
                    }
                ...
                {
                    vector<vector<int>> pre_grid = agent.grid;
                    rep(i, 4)
                    {
                        rep(j, 4)
                        {
                            agent.grid[i][j] = pre_grid[Rotate[i][j] / 4][Rotate[i][j] % 4];
                        }
                    }
                }
            }
            ...
            rep(d, 4)
            {
                ...
                rep2(times, 8)
                {
                    rep(ind, n_tuple_cnt)
                    {
                        ...
                        rep(i, 6)
                        {
                            BT_ind[i] = min(MAX2 - 1, agent2.grid[x[ind][i]][y[ind][i]]);
                        }
                        int s_v = BaseTwelve[BT_ind[0]][BT_ind[1]][BT_ind[2]][BT_ind[3]][BT_ind[4]][BT_ind[5]];
                        score += V[ind][s_v];
                    }
                    ...
                    {
                        vector<vector<int>> pre_grid = agent2.grid;
                        rep(i, 4)
                        {
                            rep(j, 4)
                            {
                                agent2.grid[i][j] = pre_grid[Rotate[i][j] / 4][Rotate[i][j] % 4];
                            }
                        }
                    }
                }
