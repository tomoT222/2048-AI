...
long long addition_score[MAX][MAX][MAX][MAX];
void make_new_line() {
    rep(a, MAX) {
        rep(b, MAX) {
            rep(c, MAX) {
                    ...
                    rep(i, 4) {
                        if (grid[i] != 0) {
                            if (v.size() != 0 && v.back() == grid[i] && !last) {
                                ...
                                addition_score[a][b][c][d] += grid[i] * 2;
                            }
    ...
    board move(int d) {
        ...
        if (d == 0) { //U
            rep(j, 4) {
                ...
                next_b.score += addition_score[grid[0][j]][grid[1][j]][grid[2][j]][grid[3][j]];
            }
        }
        if (d == 1) { //L
            rep(i, 4) {
                ...
                next_b.score += addition_score[grid[i][0]][grid[i][1]][grid[i][2]][grid[i][3]];
            }
        }
        if (d == 2) { //D
            rep(j, 4) {
                ...
                next_b.score += addition_score[grid[3][j]][grid[2][j]][grid[1][j]][grid[0][j]];
            }
        }
        if (d == 3) { //R
            rep(i, 4) {
                ...
                next_b.score += addition_score[grid[i][3]][grid[i][2]][grid[i][1]][grid[i][0]];
            }
        }
...
int main() {
    ...
    board sb;
    //sb.seed = 13339144; //xorshift() % 50515093L;
    ...
    while (1) {
        ...
        long long min_score = 1000000000000000000; board ans; vector<status> v(1, { 0, sb });
